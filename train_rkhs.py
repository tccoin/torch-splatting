...
class RSTrainer(Trainer):
    ...
    def on_train_step(self):
        ...
        with prof:
            ...
            out = self.render(pc=self.model, camera=camera)
        ...
        input_frame = self.render(pc=self.input_model, camera=camera, mode='train')
...
if __name__ == "__main__":
    ...
    gauss_model = RKHSModel(sh_degree=4, debug=False, trainable=True)
    ...
    if USE_PROFILE:
        with LineProfiler(trainer.render.render) as lp:
            try:
                trainer.train()
